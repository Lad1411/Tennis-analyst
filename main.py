...
def get_args():
    ...
    parser.add_argument("--player_detector", "-p",type=str, default="best_yolo.pt",
                        help= "Player detect model path")
    parser.add_argument("--threshold", "-t", type=float, default=0.5,
                        help="Player model confidence score threshold")
    parser.add_argument("--ball_detector", "-b", type=str, default="best_tracknet.pt",
...
def draw_ball(tracker, idx, original_frame, trace=4):
    for i in range(trace):
        if np.isnan(tracker[idx - i][0]) or np.isnan(tracker[idx - i][1]):
            return frame  # Skip drawing for this frame
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        prediction = detect_model.track(frame, persist=True)[0]
        ...
        for idx in range(len(categories)):
            ...
            if category ==  1:
                ...
                x_cen = (xmin+xmax)/2
                y_cen = (ymin+ymax)/2
                dis_to_cen = math.dist([x_cen, y_cen], [frame_cen_x, frame_cen_y])
                all_people.append([xmin, ymin, xmax, ymax, player_id, dis_to_cen])

        all_people.sort(key=lambda x: x[5])
        players = [p[:5] for p in all_people[:2]]
    ...
    lag_offset = 1  # Try changing this to 2 if it's still slightly behind
    ball_trackers[:-lag_offset] = ball_trackers[lag_offset:]

    # Duplicate the last frame's coordinate to fill the empty spots at the end of the array
    ball_trackers[-lag_offset:] = ball_trackers[-lag_offset - 1]
